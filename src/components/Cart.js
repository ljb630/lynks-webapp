    ...
    if(!cartItems?.length) {
        return (
            <div className="flex flex-col items-center justify-center pt-[100px] max-w-lg mx-auto h-screen">
                ...
                <button 
                    className="py-3 px-6 uppercase font-semibold text-white bg-[#A51515]"
                    onClick={() => navigate(`/restaurant-name/${selectedRestaurant?.id}`)}
                >
                    Browse more food items
                </button>
            </div> 
        )
    }
                            ...
                            <div key={item?.id} className="flex items-center py-2.5">
                                ...
                                <div className="flex items-center ml-4">
                                    <div className="flex border items-center justify-center w-[70px] text-sm h-8 border-[#d4d5d9]">
                                        <span 
                                            className="block text-center font-medium w-1/3 text-base cursor-pointer text-[#A51515]"
                                            onClick={() => handleRemove(item)}
                                        >
                                            -
                                        </span>
                                        <span className="block text-center font-medium w-1/3 text-[#A51515] ">{item?.itemCount}</span>
                                        <span 
                                            className="block text-center font-medium w-1/3 text-[#A51515] cursor-pointer text-base"
                                            onClick={() => handleAdd(item)}
                                        >
                                            +
                                        </span>
                                    </div>
                                    <div className="ml-5 text-[13px] text-[#606375] text-right w-[60px]">${parseFloat((item?.itemCount * item?.price))}</div>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="w-3/12 bg-white self-start min-h-[400px] mt-8 ml-0 mr-10 pt-5 px-8  ">
                <div className="pb-5 border-b-2 border-[#282c3f]">
                    ...
                    <div className="flex items-center text-[13px] font-light my-2.5">
                        ...
                        <div className="flex-1 text-right">${5}</div>
                    </div>
                ...
                <div className="flex items-center pt-6 text-[#282c3f] font-semibold">
                    ...
                    <div className="flex-1 text-right">${(totalAmount+40).toFixed(2)}</div>
                </div>

                <div className="h-12 text-base flex font-semibold mt-10">
                    <button className="cursor-pointer text-[#A51515] border-2
                        border-[#A51515] flex-grow uppercase" onClick={handleGoBack}>Go Back</button>
                    <span className="w-5"></span>
                    <button className="cursor-pointer text-[#fff] bg-[#A51515] 
                        flex-grow uppercase"
